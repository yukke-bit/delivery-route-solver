...
class VRPVisualizer:
    ...
    def plot_solution(self, routes: List[List[int]], title: str = "VRP解の可視化", 
    ...
    def plot_solution_comparison(self, routes1: List[List[int]], routes2: List[List[int]],
        ...
        self.plot_solution(routes1, 
                          title=f"{label1} (コスト: {cost1:.2f})", 
                          show_plot=False)
        
        # 第2の解
        plt.sca(ax2) 
        self.plot_solution(routes2, 
                          title=f"{label2} (コスト: {cost2:.2f})", 
                          show_plot=False)
        
        # 全体のタイトル
        fig.suptitle(f'VRP解の比較: {self.instance.name}', fontsize=16, fontweight='bold')
    ...
    def plot_problem_instance(self, save_path: str = None, show_plot: bool = True) -> None:
        ...
        plt.title(f'VRP問題インスタンス: {self.instance.name}', fontsize=16, fontweight='bold')
        ...
        plt.scatter([], [], c='lightblue', s=100, alpha=0.7, 
                   label='顧客 (サイズ=需要量)', edgecolor='navy')
        ...
        # 問題情報を表示
        info_text = f"顧客数: {len(self.customers)}\n車両容量: {self.instance.capacity}"
        ...
        if save_path:
            plt.savefig(save_path, dpi=300, bbox_inches='tight')
            print(f"問題インスタンスを保存しました: {save_path}")
